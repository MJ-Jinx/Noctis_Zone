...
| `bonding_curve.compact` | Quadratic price discovery (shared `k` for Tier B/C), fee routing, graduation, plus the full eligibility/DarkVeil merge for Tier C (2026-07-10); same 20-level Merkle allowlist proof as above | ✅ compiles clean (full ZK) |
| `creator_escrow.compact` | Creator fee escrow only (1.0% trade fees, monthly claim) — no vesting. Never actually holds a real fee for either tier in the current architecture — real Tier B/C fee accrual happens elsewhere, see the contract's own file header | ✅ compiles clean (full ZK) |
...
│ darkveil + bonding_curve, folded together 2026-07-10) │
...
│ (fee escrow claim, │ │ ADA/NIGHT split) │ │ NO withdraw — by │
